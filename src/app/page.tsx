...
// --- COMPONENT IMPORT ---
import { ProjectCard, ProjectCardProps } from "../../component/ProjectCard"; // Import the type too
...
// --- TYPES FOR SCROLLER ---
interface TechItem {
  src: StaticImageData | string; // Allow string for SVGs if needed
  alt: string;
}

// --- INFINITE SCROLLER COMPONENT ---
const InfiniteScroller = ({ items }: { items: TechItem[] }) => { // Apply the TechItem type
...
export default function Home() {
  ...
  const techStack: TechItem[] = [ // Apply the TechItem type to the array
  ...
  const featuredProject: ProjectCardProps = { // Apply the ProjectCardProps type
    ...
    <main className="flex flex-col bg-black">
            ...
            <motion.div variants={fadeIn(0.5)} initial="hidden" animate="visible" className="md:col-span-4 flex justify-center items-center">
              <div className="relative group w-[250px] h-[250px] sm:w-[300px] sm:h-[300px] md:w-[350px] md:h-[350px]">
                ...
                <Image
                  ...
                  alt="Ankit Shukla"
                  fill
                  sizes="(max-width: 768px) 250px, 350px" // Recommended for performance
                  className="relative object-cover rounded-full border-4 border-gray-800"
                  priority
                />
              </div>
            </motion.div>
      ...
      <section className="py-24 bg-gray-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          {isMounted && <InfiniteScroller items={techStack} />}
        </div>
      </section>
